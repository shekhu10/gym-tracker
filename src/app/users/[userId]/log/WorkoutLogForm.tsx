...
export function WorkoutLogForm({
  ...
  const progressPercentage =
    log.exercises.length > 0
      ? (completedExercises / log.exercises.length) * 100
      : 0;
    ...
    <div className="space-y-6">
        ...
        <div className="mb-4">
          ...
          <div className="mt-3">
            <div className="flex justify-between text-sm text-gray-300 mb-2">
              <span>
                Progress: {completedExercises} of {log.exercises.length}{" "}
                exercises completed
              </span>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-white">
          Exercise {currentExerciseIdx + 1} of {log.exercises.length}
        </h3>
        {log.exercises.length > 0 ? (
          <ExerciseLogEditor
            ...
            previousWeekExercise={
              previousWeekLog?.entries?.exercises?.[currentExerciseIdx]
            }
            onChange={(updatedExercise) =>
              updateExercise(currentExerciseIdx, updatedExercise)
            }
          />
        ) : (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3 text-sm">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                updateExercise({
                  actualRestBetweenRounds: Number(e.target.value) || 0,
                })
              }
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-300">
                  Target sets:{" "}
                  {planExercise?.type === "circuit"
                    ? planExercise.exercises.reduce(
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-300">
                  Last week sets:{" "}
                  {previousWeekExercise?.exercises?.reduce(
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-300">
                  Will save:{" "}
                  {exercise.exercises.reduce((total: number, ex: any) => {
                    return (
                      total +
                      ex.sets.filter((set: any) => {
                        if (set.type === "strip") {
...
function SingleExerciseLogEditor({
  exercise,
  planExercise,
  previousWeekExercise,
  onChange,
  isInCircuit = false,
}: SingleExerciseLogEditorProps) {
  ...
  return (
    <div className={`border rounded-lg p-4 ${bgColor} ${borderColor}`}>
      ...
      {!isInCircuit && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3 text-sm">
          <div>
            ...
            <span className="text-gray-400">
              {exercise.restBetweenSets ||
                (planExercise && "restBetweenSets" in planExercise
                  ? planExercise.restBetweenSets
                  : "N/A")}{" "}
              sec
            </span>
          </div>
          <div>
            ...
            <span className="text-gray-400">
              {exercise.restAfterExercise ||
                (planExercise && "restAfterExercise" in planExercise
                  ? planExercise.restAfterExercise
                  : "N/A")}{" "}
              sec
            </span>
          </div>
        ...
        <div className="bg-gray-800 border border-gray-600 rounded p-3 mb-3">
          <div className="flex flex-wrap gap-4 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300">
                Target sets:{" "}
                {planExercise && "sets" in planExercise
      ...
      <Button variant="secondary" size="sm" onClick={addSet} className="mt-2">
        + Add Set
      </Button>
    </div>
  );
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 p-2 border border-gray-600 rounded bg-gray-900">
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto">
        <input
          ...
          value={set.reps || ""}
          onChange={(e) =>
            onChange({ ...set, reps: Number(e.target.value) || 0 })
          }
        ...
        <input
          ...
          value={set.weight || ""}
          onChange={(e) =>
            onChange({ ...set, weight: Number(e.target.value) || 0 })
          }
      ...
      <div className="flex flex-col gap-1 w-full sm:w-auto">
        ...
        {(previousWeekSet || set.lastWeekData) && (
          <div className="text-xs text-blue-400">
            Last week: {(previousWeekSet || set.lastWeekData)?.reps}×
            {(previousWeekSet || set.lastWeekData)?.weight}
          </div>
        )}
