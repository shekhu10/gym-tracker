...
export async function POST(req: Request, { params }: Context) {
  ...
  if (body.status && !statusValues.includes(body.status)) {
    return NextResponse.json({ error: `Invalid status. Allowed: ${statusValues.join(', ')}` }, { status: 400 });
  }
  if (body.source && !sourceValues.includes(body.source)) {
    return NextResponse.json({ error: `Invalid source. Allowed: ${sourceValues.join(', ')}` }, { status: 400 });
  }
